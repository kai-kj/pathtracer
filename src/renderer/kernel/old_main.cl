...
float3 random_unit_vector(ulong *rng) {
	...
	float theta = rand_float_in_range(rng, 0, 2 * PI);
...
float3 get_color(Renderer *r, Ray ray) {
	...
	for(int i = 0; i < maxDepth; i++) {
		...
		if(cast_ray(r, ray, &voxel, &hitPos, &iNormal, &material)) {
			...
			// return -fNormal.x * 0.5f + 0.5f;
			// return -fNormal * 0.5f + 0.5f;
			return convert_float3(voxel) / 10;
			// if(i == 0)
			...
			if(material.type == 1) {
				...
				ray = (Ray){
					hitPos,
					normalize(reflection_dir(ray.direction, fNormal) + random_unit_vector(r->rng) * material.details.metal.fuzz)
				};
				voxel += iNormal;
